...
Begin
  // Ошибка исключений понадобилась ввиду того, что Locate может попытаться позиционировать
  // курсор на строку, которая скрыта при помощи методоа onFiterRecord. При этом возникает Run-Time Ошибка
  // "Душим" ошибку при помощи try..except
  try
  case Panel of
